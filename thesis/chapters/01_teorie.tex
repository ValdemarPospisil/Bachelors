...
\chapter{Architektura operačního systému Linux}
\label{chap:linux_arch}

Tato kapitola analyzuje klíčové mechanismy operačního systému Linux, které jsou nezbytné pro běh systémových služeb a správu síťových rozhraní.

\section{Struktura systému: User Space a Kernel Space}
...
Aplikace v uživatelském prostoru nemohou přímo přistupovat k hardwaru. Musí využívat tzv. \textit{systémová volání} (syscalls) pro komunikaci s jádrem. V kontextu VPN klienta je typickým příkladem požadavek na vytvoření virtuálního síťového rozhraní.

\section{Konfigurace síťového subsystému (Netlink)}
...
\begin{enumerate}
    ...
    \item \textbf{Bezpečnost a řízení přístupu:} Přístup k socketu lze omezit standardními linuxovými oprávněními souborů (viz sekce \ref{sec:permissions}). To umožňuje, aby se k backendové službě mohl připojit pouze uživatel s příslušným oprávněním.
\end{enumerate}

\section{Správa oprávnění a souborový systém}
...
\subsection{Model oprávnění DAC}
...
Tento mechanismus je klíčový pro uložení citlivých dat (např. konfiguračních souborů s klíči). Pokud je konfigurační soubor nastaven s právy `600` (čtení/zápis pouze pro vlastníka) a vlastníkem je `root`, žádný jiný uživatel v systému nemůže tato data přečíst.

\subsection{Umístění aplikace}
Pro software třetích stran, který není součástí oficiální distribuce, vyhrazuje FHS adresář \texttt{/opt}. Konfigurační soubory a logy jsou standardně umisťovány do \texttt{/etc}, respektive \texttt{/var/log}.\section{Implementační technologie}
