...
Linux využívá model Discretionary Access Control (DAC). Každý soubor má vlastníka (User), skupinu (Group) a sadu práv pro čtení, zápis a spuštění (`rwx`).
Tento mechanismus je klíčový pro uložení citlivých dat (např. konfiguračních souborů s klíči). Pokud je konfigurační soubor nastaven s právy \texttt{600} (čtení/zápis pouze pro vlastníka) a vlastníkem je \texttt{root}, žádný jiný uživatel v systému nemůže tato data přečíst.
