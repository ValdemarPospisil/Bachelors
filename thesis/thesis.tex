...
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, shapes.symbols, positioning, babel}
\usepackage{float}
\usepackage{color}
\usepackage{array}
...
\begin{flushright}
    Děkuji \vedouciDAT\ za odborné vedení a cenné rady při tvorbě této práce.
    Poděkování patří také kolegům z týmu GoodAccess za technickou podporu
    a inspirativní konzultace.
\end{flushright}
...
\textbf{Klíčová slova:} VPN, Zero Trust, rozhraní příkazové řádky, WireGuard, Linux
...
\textbf{Keywords:} VPN, Zero Trust, command-line interface, WireGuard, Linux
...
\sloppy
\printbibliography[heading=bibintoc, title=Seznam použitých zdrojů]

\appendix

\include{chapters/07_prilohy}
\end{document}
