...
- [ ] Task: Write the `\subsection{Implementace zabezpečeného úložiště (Secure Storage)}` content, including a figure environment for `thesis/images/security-component.png`.
- [ ] Task: Write the `\subsection{Bezpečnost meziprocesové komunikace (IPC)}` content based on the specification.
- [ ] Task: Review the drafted section to ensure smooth flow, correct LaTeX syntax, and integration with the rest of the chapter.
